  ...
  for (;;) {
    ...
    if (client) {
      ...
      if (getLocalTime(&timeinfo)) {
        ...
        // Prepare HTML response with CSS for better styling
        String response = "<!DOCTYPE html><html><head>";
        response += "<style>";
        response += "body { font-family: Arial, sans-serif; background-color: #f0f0f0; color: #333; text-align: center; padding: 20px; }";
        response += "h1 { color: #0066cc; }";
        response += ".sensor-data { background-color: #fff; padding: 20px; margin: 20px auto; border-radius: 10px; box-shadow: 0px 0px 10px rgba(0,0,0,0.1); max-width: 600px; }";
        response += "p { font-size: 18px; }";
        response += ".timestamp { font-size: 16px; color: #777; }";
        response += "</style>";
        response += "</head><body>";

        // Add title and sensor data in HTML
        response += "<h1>Sensor Data</h1>";
        response += "<div class='sensor-data'>";
        response += "<p class='timestamp'>Last Update: " + String(timeString) + "</p>";
        response += "<p><strong>SHT4X Temperature:</strong> " + String(temp_sht4x) + " °C</p>";
        response += "<p><strong>SHT4X Humidity:</strong> " + String(humid_sht4x) + " %</p>";
        response += "<p><strong>BMP280 Temperature:</strong> " + String(temp_bmp280) + " °C</p>";
        response += "<p><strong>BMP280 Pressure:</strong> " + String(pressure_bmp280) + " hPa</p>";
        response += "<p><strong>MPU6050 Acceleration:</strong> X=" + String(accX) + " m/s², Y=" + String(accY) + " m/s², Z=" + String(accZ) + " m/s²</p>";
        response += "<p><strong>MPU6050 Gyroscope:</strong> X=" + String(gyroX) + " rad/s, Y=" + String(gyroY) + " rad/s, Z=" + String(gyroZ) + " rad/s</p>";
        response += "<p><strong>MPU6050 Temperature:</strong> " + String(temp_mpu6050) + " °C</p>";
        response += "</div>";
    ...
    vTaskDelay(pdMS_TO_TICKS(200)); // Check for client every 0.2 second
  }
}
