...
void webServerTask(void *parameter) {
  ...
  for (;;) {
    ...
    if (client) {
      ...
      // Format the time correctly using sprintf
      char formattedTime[9]; // HH:MM:SS
      ...
      response += "<title>Sensor Data</title>";
      response += "<style>";
      response += "body { font-family: Arial, sans-serif; background-color: #f4f4f4; margin: 20px; }";
      response += "h1 { color: #333; }";
      response += "p { font-size: 18px; color: #555; }";
      response += ".sensor { background-color: #fff; border: 1px solid #ccc; border-radius: 5px; padding: 10px; margin-bottom: 10px; }";
      response += "</style>";
      response += "</head>";
      response += "<body>";
      response += "<h1>Sensor Data</h1>";
      response += "<div class='sensor'><p><strong>Last Update:</strong> " + String(formattedTime) + "</p></div>";
      response += "<div class='sensor'><p><strong>SHT4X Temperature:</strong> " + String(temp_sht4x) + " °C</p></div>";
      response += "<div class='sensor'><p><strong>SHT4X Humidity:</strong> " + String(humid_sht4x) + " %</p></div>";
      response += "<div class='sensor'><p><strong>BMP280 Temperature:</strong> " + String(temp_bmp280) + " °C</p></div>";
      response += "<div class='sensor'><p><strong>BMP280 Pressure:</strong> " + String(pressure_bmp280) + " hPa</p></div>";
      response += "<div class='sensor'><p><strong>MPU6050 Acceleration:</strong> X=" + String(accX) + " m/s², Y=" + String(accY) + " m/s², Z=" + String(accZ) + " m/s²</p></div>";
      response += "<div class='sensor'><p><strong>MPU6050 Gyroscope:</strong> X=" + String(gyroX) + " rad/s, Y=" + String(gyroY) + " rad/s, Z=" + String(gyroZ) + " rad/s</p></div>";
      response += "<div class='sensor'><p><strong>MPU6050 Temperature:</strong> " + String(temp_mpu6050) + " °C</p></div>";
      response += "</body></html>";
    ...
    vTaskDelay(pdMS_TO_TICKS(100)); // Check for client every 0.1 seconds
  }
}
